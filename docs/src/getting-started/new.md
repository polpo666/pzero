...
```bash
docker run --rm -v ${PWD}:/app ghcr.io/polpo666/pzero:latest new your_project --frame api
cd your_project
# 下载依赖
go mod tidy
# 启动服务端程序
...
```bash
docker run --rm -v ${PWD}:/app ghcr.io/polpo666/pzero:latest new your_project --frame rpc
cd your_project
# 下载依赖
go mod tidy
# 启动服务端程序
...
```bash
docker run --rm -v ${PWD}:/app ghcr.io/polpo666/pzero:latest new your_project --frame gateway
cd your_project
# 下载依赖
go mod tidy
# 启动服务端程序
